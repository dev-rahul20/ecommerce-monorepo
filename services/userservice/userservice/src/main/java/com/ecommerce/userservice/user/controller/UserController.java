...
public class UserController {
	...
	public UserResponse createUser(@RequestBody @Valid UserRequestDto user) {
		...
		return new UserResponse(true, HttpStatus.CREATED, "User Craeted Successfully!!", craeteUserId);
	}

	@GetMapping
	public UserResponse getAllUsers() {
		List<UserResponceDto> list = service.getAllUsers();
    ...
    public UserResponse getUserByUserId(@PathVariable @NotNull @Positive Integer userId) {
        UserResponceDto user = service.getByUserId(userId);
        return new UserResponse(true, HttpStatus.OK, "User found Successfully!!", user);
    }
	
	@PutMapping("/{userId}")
    public UserResponse updateUser(@PathVariable @Valid @Positive Integer userId, @RequestBody @Valid UserResponceDto user) {
